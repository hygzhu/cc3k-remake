...
Point::Point(const Point &other)
    : m_x(other.m_x), m_y(other.m_y) {} // copy constructor
...
bool Point::doLinesIntersect(Point p1, Point p2, Point q1, Point q2) {
  ...
  if (x >= std::min(p1.getX(), p2.getX()) &&
      x <= std::max(p1.getX(), p2.getX()) &&
      x >= std::min(q1.getX(), q2.getX()) &&
      x <= std::max(q1.getX(), q2.getX()) &&
      y >= std::min(p1.getY(), p2.getY()) &&
      y <= std::max(p1.getY(), p2.getY()) &&
      y >= std::min(q1.getY(), q2.getY()) &&
      y <= std::max(q1.getY(), q2.getY())) {
    return true;
  }
...
std::vector<Point> Point::getSurroundingPointsNAway(int n) {
  std::vector<Point> neighbours;
  int x = m_x, y = m_y;
  neighbours.push_back(Point(x, y + n)); // up
  neighbours.push_back(Point(x, y - n)); // down
  neighbours.push_back(Point(x - n, y)); // left
  neighbours.push_back(Point(x + n, y)); // right
  neighbours.push_back(Point(x + n, y + n));
  neighbours.push_back(Point(x + n, y - n));
  neighbours.push_back(Point(x - n, y + n));
  neighbours.push_back(Point(x - n, y - n));
  return neighbours;
