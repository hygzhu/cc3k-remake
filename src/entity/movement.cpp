...
Point Movement::getPoint(){
    return Point(m_x, m_y);
}

bool Movement::collidable(){
    return m_collidable;
}
...
void Movement::setInvincible(bool i){
    m_isInvincible = i;
}



void Movement::tryToMove(double time, std::vector<std::shared_ptr<Entity> > otherEntities)
{
    tick(time);
...
bool Movement::isThereCollisionAtDestinationPoint(Point p, std::vector<std::shared_ptr<Entity> > otherEntities){
    ...
    for(const auto& otherEntity : otherEntities)
    {
        ...
        if (otherEntity->getHitbox().collidesWith(hitbox)) {
            ...
            if(otherEntity->getMovement()->collidable() && otherEntity->getHitbox().surrounds(hitbox)){
...
Point Movement::closestMovablePoint(Point p, std::vector<std::shared_ptr<Entity> > otherEntities){
    int oldx = m_x;
    int oldy = m_y;
